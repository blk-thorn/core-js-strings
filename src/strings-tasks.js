...
function sumOfCodes(str) {
  if (str) {
    ...
    const lettersArray = strSplit.map((el) => el.charCodeAt());
    const sumOfNumbers = lettersArray.reduce((acc, number) => acc + number);
    return sumOfNumbers;
  }
  return 0;
}
...
function formatTime(minutes, seconds) {
  const changedMinutes = Math.abs(minutes).toString().padStart(2, '0');
  const changedSeconds = Math.abs(seconds).toString().padStart(2, '0');
...
function isPalindrome(str) {
  ...
  const cleanedStrLen = cleanedStr.length;

  let left = 0;
  let right = cleanedStrLen - 1;
...
function encodeToRot13(str) {
  return str.replace(/[A-Za-z]/g, (letter) => {
    return String.fromCharCode(
      letter.charCodeAt(0) + (letter.toUpperCase() <= 'M' ? 13 : -13)
    );
  });
}
